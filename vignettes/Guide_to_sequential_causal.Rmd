---
title: "A guide to the `sequential.causal` package"
...
Let's jump into a simple example. Suppose that we wish to estimate the ATE in an observational setting with no unmeasured confounding.
...
\]
where $\epsilon_i \iid t_{4}$ are drawn from a $t$-distribution with 4 degrees of freedom. We now describe the three models used to estimate $\psi$ knowing the distribution of $A_1, \dots, A_n$ but without any knowledge of $f^\star$ or the distribution of $\epsilon_i$.
