...
load('../computations/start_time/miscoverage_rates.RData')

n <- as.integer(names(miscoverage_rates)[[1]]) +
  length(miscoverage_rates[[1]]) - 1

load('../computations/start_time/alpha.RData')
...
```{r}
load('../computations/cs_vs_ci/cs_vs_ci.RData')
...
t = unique(round(logseq(x1 = start_time, x2 = n, n = 10000)))
...
load('../computations/randomized_experiment_SL/randomized_experiment_SL.RData')
...
load('../computations/observational_study_SL/observational_study_SL.RData')
...
load('../computations/sepsis/sepsis_cs.RData')
...
plt_cs <- 
  ...
  geom_line(aes(x=t, y=bound, color=Model, linetype=Model), 
            data=plot_df[plot_df$upper_lower=='u',]) +
  ylab('Estimated effect of high fluid intake on 30-day mortality') +
